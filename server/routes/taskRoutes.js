...
const { createTask, getTasks, getAllTasks,  updateTask, deleteTask} = require("../controllers/taskController"); // Import task controller functions
...
router.put("/:id", protect, updateTask); // Define a route for updating a task by ID, which calls the updateTask function from taskController
router.delete("/:id", protect, deleteTask); // Define a route for deleting a task by ID, which calls the deleteTask function from taskController
